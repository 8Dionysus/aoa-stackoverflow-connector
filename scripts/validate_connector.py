#!/usr/bin/env python3
"""Validate the GitHub-publishable StackOverflow connector skeleton."""
...
COMMAND_LINE_RE = re.compile(
    ...
    r"aoa-stackoverflow(?:\s|$)|pip(?:3)?\s+|uv\s+|git\s+|export\s+)",
    re.IGNORECASE,
)
...
REQUIRED_FILES = [
    ...
    "connector/profiles/python-datetime-timezone.yaml",
    "connector/seeds/python_datetime_timezone_questions.yaml",
    ...
    "docs/CROSS_CONNECTOR_GENERALIZATION_REPORT.md",
    "docs/decisions/README.md",
    "docs/decisions/AOA-STACKOVERFLOW-D-0001-third-source-connector-proof.md",
    "evals/AGENTS.md",
    "evals/PORT.yaml",
    ...
    "stats/packets/public-fixture-source-record-evidence-materialization-ratio.reference.json",
    "src/aoa_stackoverflow_connector/cli.py",
]
...
REQUIRED_DIRS = [
    ...
    "src/aoa_stackoverflow_connector/parse",
    "src/aoa_stackoverflow_connector/normalize",
    "src/aoa_stackoverflow_connector/index",
    "src/aoa_stackoverflow_connector/graph",
    "src/aoa_stackoverflow_connector/claims",
    "src/aoa_stackoverflow_connector/answer",
    "src/aoa_stackoverflow_connector/query",
    "src/aoa_stackoverflow_connector/evaluation",
    "src/aoa_stackoverflow_connector/storage",
    "tests/unit",
    "tests/contract",
    "tests/integration",
    "evals/intake",
...
REQUIRED_SCHEMAS = [
    ...
    "normalized_question.schema.json",
    "normalized_answer.schema.json",
    "normalized_comment.schema.json",
    "normalized_item.schema.json",
...
def main() -> int:
    ...
    payload = {
        "schema": "aoa_stackoverflow_connector_validation_v1",
...
def _check_text(repo_root: Path, errors: list[str], warnings: list[str]) -> None:
    ...
    for token in ["login", "account", "search", "write", "answer", "comment", "internal search"]:
        if token not in source_policy.casefold():
            errors.append(f"source policy missing denied token: {token}")

    for token in ["/users/login", "/users/current", "/search", "/questions/ask", "/answer", "/comment", "internal-search"]:
    ...
    for token in ["claim", "claim_relation", "insufficient_evidence", "warning_report", "freshness_report", "accepted answer", "score"]:
...
def _check_eval_port(repo_root: Path, errors: list[str]) -> None:
    ...
    for token in [
        ...
        "owner_repo: aoa-stackoverflow-connector",
    ...
    expected_suites = {
        "starter_claim_conflict_relations.json": "aoa_stackoverflow_claim_graph_eval_suite_v1",
        "starter_claim_answer_packets.json": "aoa_stackoverflow_answer_eval_suite_v1",
    }
