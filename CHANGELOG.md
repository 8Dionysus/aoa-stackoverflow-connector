...
### Consolidated canonical release

This is the single canonical source release for the same-day campaign. It
consolidates the three superseded published release snapshots (`v0.1.0`,
`v0.1.1`, and `v0.1.2`) without discarding their capability, schema/ABI,
owner/routing, security/privacy, operator, generated, validation, limitation,
non-claim, or publication-history material. The historical tag-scoped snapshot
is retained verbatim below as quoted text; the line-level conservation ledger
records every source line and its disposition.

The active source contract is `0.1.0` / `v0.1.0`. The final provider bindings
are `aoa-kag@v0.5.0` at `f46f146cc79a26fa81ad0f400b9c5774df293e57`, including the
`repo-local-kag-index` action at that same commit, and `aoa-stats@v0.2.0` at
`88ff38b1b38eef939f2c5b4541cbe8363a05fc8d`. These are source/validation
bindings only; provider release identity and admission remain separate owner
claims.

### Current source, owner, and claim boundaries

- This is a source-only connector release: method, code, schemas, small
  sanitized fixtures, local validation, and bounded KAG/stats owner surfaces.
- The connector remains read-only and policy-gated. It does not publish a full
  StackOverflow corpus, activate a crawler, expose account/private/write routes,
  deploy or serve the runtime MCP, or own central proof or human acceptance.
- CI and owner-local checks prove only their declared source, generated-family,
  provider-pin, stats-port, test, and release-contract scopes. They do not prove
  artifact admission, package trust, runtime deployment/health, transport
  delivery, semantic proof, downstream acceptance, closure, or owner acceptance.
- The final release body is this canonical `CHANGELOG.md` section. No package
  or other release asset is implied.

### Merged history after the original 14/14 reconciliation

The live first-parent history is reconciled as 17/17 entries. The three
post-initial release entries are retained here with their owner and generated
material; their former patch versions are historical snapshots, not active
version markers.

- Commit `142abe89b5fc8e2b4e5728fe532067087ab80149`, PR #13, “release:
  aoa-stackoverflow-connector v0.1.0”: owner release route, source version
  contract, provider declarations, release dry-run/publish/audit gates, release
  documentation, and generated KAG refresh.
- Commit `35cc282d4b2ae57767890965ee6d8d3f7df6ddb7`, PR #14, “fix(release):
  revalidate aoa-stats v0.2.1”: a source/validation-plane provider correction,
  exact stats pin, release-contract/test updates, and the generated KAG refresh.
  Its superseded provider identity and validation/non-claim text remain in the
  historical snapshot and ledger.
- Commit `5533d212e92b740ad3e6bf989c511027b451c049`, PR #15, “fix(release):
  revalidate exact aoa-kag v0.5.2 provider”: exact KAG/stats provider and action
  identity separation, generated-family refresh, source-only release-contract
  updates, and explicit artifact/runtime/proof/delivery/closure/acceptance
  limits. Its superseded identities remain historical; the active bindings are
  the final provider pins stated above.

### Historical tag-scoped CHANGELOG snapshot (verbatim, pre-cleanup)

The following quoted block is the exact `CHANGELOG.md` text captured at the
pre-cleanup boundary from the `v0.1.2` tag. Quoting prevents its historical
`##` headings from creating additional active release sections while preserving
every line, including formatting and historical version/provider statements.
