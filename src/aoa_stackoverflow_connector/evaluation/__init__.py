"""Local eval suite runners for the StackOverflow connector."""
...
from aoa_stackoverflow_connector.query import query_graph_packet


SOURCE_URL = "https://stackoverflow.com/questions/fixture-python-datetime-timezone-aware-utc"


def ensure_fixture_run(run_id: str = "starter-fixture", profile_id: str = "python-datetime-timezone") -> dict[str, Path]:
...
def run_claim_relation_suite(suite_path: Path, repo_root: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema": "aoa_stackoverflow_claim_graph_eval_report_v1",
...
def run_answer_packet_suite(suite_path: Path, repo_root: Path | None = None) -> dict[str, object]:
    ...
    return {
        "schema": "aoa_stackoverflow_answer_eval_report_v1",
        "suite_id": suite.get("suite_id"),
        "status": "pass" if not failures else "fail",
        "failures": failures,
        "cases": case_reports,
...
def _check_answer_expectations(case_id: str, answer: dict[str, object], expect: dict[str, object]) -> list[str]:
    ...
    if expect.get("requires_item_ids") and not any(step.get("item_id") for step in answer.get("evidence_chain", [])):
        failures.append(f"{case_id}: missing item ids")
    if expect.get("requires_answer_ids") and not any(step.get("answer_id") for step in answer.get("evidence_chain", [])):
        failures.append(f"{case_id}: missing answer ids")
    ...
    if int(expect.get("min_conflicting_claims", 0)) > len(conflict.get("conflicting_claim_ids", [])):
        failures.append(f"{case_id}: too few conflicting claims")
    score_report = answer.get("score_signal_report", {})
    if expect.get("requires_accepted_signal") and not score_report.get("accepted_answer_item_ids"):
        failures.append(f"{case_id}: missing accepted answer signal")
    if expect.get("requires_score_signal") and not score_report.get("scored_item_count"):
        failures.append(f"{case_id}: missing score signal")
