"""Command line interface for the StackOverflow connector."""
...
from aoa_stackoverflow_connector.answer import render_answer_packet
from aoa_stackoverflow_connector.config import StorageRoots, find_repo_root
from aoa_stackoverflow_connector.evaluation import SOURCE_URL, run_answer_packet_suite, run_claim_relation_suite
from aoa_stackoverflow_connector.graph import build_graph
from aoa_stackoverflow_connector.index import build_keyword_index
...
from aoa_stackoverflow_connector.query import query_graph_packet, query_keyword_index
from aoa_stackoverflow_connector.sources import (
    ACCESS_MODES,
    MEDIA_POLICIES,
    SOURCE_KINDS,
    build_source_plan,
...
from aoa_stackoverflow_connector.storage import create_storage_roots, storage_status


DEFAULT_PROFILE = "python-datetime-timezone"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="aoa-stackoverflow")
    ...
    sources = sub.add_parser("sources", help="Manage operator-local StackOverflow sources.")
    ...
    sources_plan.add_argument("--run", default="stackoverflow-owned-sources")
    sources_plan.add_argument("--limit", type=int, default=50)
...
def cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    required = [
        ...
        "connector/fixtures/stackexchange/python_datetime_timezone_question_bundle.json",
    ]
    missing = [rel for rel in required if not (repo_root / rel).exists()]
    payload = {
        "schema": "aoa_stackoverflow_doctor_v1",
...
def cmd_init(_args: argparse.Namespace) -> int:
    ...
    _emit({"schema": "aoa_stackoverflow_init_v1", "status": "ok", "created": created, "network_touched": False})
    return 0
...
def cmd_sources_add(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        _emit({"schema": "aoa_stackoverflow_source_registry_receipt_v1", "status": "error", "error": str(exc), "network_touched": False, "read_only": True})
        return 2
    _emit({"schema": "aoa_stackoverflow_source_registry_receipt_v1", "status": "ok", "state": state, "registry_path": str(path), "source": source, "network_touched": False, "read_only": True})
    return 0
...
def cmd_sources_list(args: argparse.Namespace) -> int:
    ...
    _emit({"schema": "aoa_stackoverflow_source_registry_list_v1", "status": "ok", "registry_path": str(registry_path(roots.data)), "source_count": len(registry.get("sources", [])), "selected_count": len(selected), "sources": selected, "network_touched": False, "read_only": True})
    return 0
...
def cmd_policy_check(_args: argparse.Namespace) -> int:
    samples = [
        SOURCE_URL,
        "https://stackoverflow.com/search?q=python+datetime",
        "https://stackoverflow.com/users/login",
        "https://stackoverflow.com/questions/ask",
        "https://api.stackexchange.com/2.3/questions/999/answers?site=stackoverflow",
    ]
    decisions = [route_decision(url) for url in samples]
    ok = decisions[0]["allowed"] is True and all(item["allowed"] is False for item in decisions[1:4]) and decisions[4]["allowed"] is True
    _emit({"schema": "aoa_stackoverflow_policy_check_v1", "status": "ok" if ok else "error", "decisions": decisions, "network_touched": False})
...
def cmd_materialize_fixture(args: argparse.Namespace) -> int:
    ...
    create_storage_roots(roots)
    fixture = repo_root / "connector" / "fixtures" / "stackexchange" / "python_datetime_timezone_question_bundle.json"
    ...
    receipt = {
        "schema": "aoa_stackoverflow_materialize_receipt_v1",
        "run_id": args.run,
...
def cmd_build_index(args: argparse.Namespace) -> int:
    ...
    receipt = {
        "schema": "aoa_stackoverflow_index_manifest_v1",
        "run_id": args.run,
        "profile_id": args.profile,
...
def cmd_build_graph(args: argparse.Namespace) -> int:
    ...
    receipt = {
        "schema": "aoa_stackoverflow_graph_receipt_v1",
        "run_id": args.run,
        "profile_id": args.profile,
...
def _missing_roots(roots: StorageRoots) -> int:
    _emit({"schema": "aoa_stackoverflow_missing_storage_roots_v1", "status": "error", "missing": roots.missing(), "network_touched": False})
    return 1
