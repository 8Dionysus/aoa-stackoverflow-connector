"""Operator-local StackOverflow source registry and fetch planning."""
...
SOURCE_KINDS = {"question", "tag", "linked_question", "stackprinter_question"}
ACCESS_MODES = {"public", "api_visible"}
MEDIA_POLICIES = {"none", "thumbnails", "documents", "all"}
REGISTRY_SCHEMA = "aoa_stackoverflow_source_registry_v1"
SOURCE_SCHEMA = "aoa_stackoverflow_source_v1"
PLAN_SCHEMA = "aoa_stackoverflow_source_fetch_plan_v1"


def registry_path(data_root: Path) -> Path:
    return data_root / "sources" / "stackoverflow_sources.json"
...
def load_registry(data_root: Path) -> dict[str, object]:
    ...
    if registry.get("schema") != REGISTRY_SCHEMA:
        raise ValueError(f"unsupported StackOverflow source registry schema: {registry.get('schema')}")
    return registry
...
def upsert_source(
    ...
    *,
    ...
    kind = kind.strip()
    access = access or ("api_visible" if kind == "tag" else "public")
    ...
    source = {
        ...
        "route": f"stackoverflow:{kind}:{source_ref}",
        "kind": kind,
        "access": access,
...
def _validate(source_ref: str, kind: str, access: str, include_media: str, trust_score: float | None) -> None:
    ...
    if kind not in SOURCE_KINDS:
        raise ValueError(f"unsupported StackOverflow source kind: {kind}")
    if access not in ACCESS_MODES:
        raise ValueError(f"unsupported StackOverflow access mode: {access}")
